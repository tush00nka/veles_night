...
impl MetadataHandler {
    ...
    pub fn load_save(&mut self){
        let filenames = fs::read_dir(SAVE_METADATA_PATH).unwrap();
        ...
        for filename in filenames{
            ...
            path = file.file_name().into_string().unwrap().split('.').next().unwrap().to_string();            
        ...
        let path_to_save = if path == "-1"{
            ...
            SAVE_METADATA_PATH.to_string() + &path + &METADATA_EXTENSION.to_string()
        };
    ...
    pub fn save(&self, level_number: u8){
        let Ok(filenames) = fs::read_dir(SAVE_METADATA_PATH) else{
        ...
        let path_to_save = SAVE_METADATA_PATH.to_string() + &level_number.to_string() + &METADATA_EXTENSION.to_string();
    ...
    pub fn change_spirits(
        ...
        for spirit in spirits_handler.spirits.values(){
            ...
            self.spirits.push(SpiritMetadata { position, amount: 1, direction});
        }
    }
